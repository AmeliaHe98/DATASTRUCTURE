...
private:
   ...
   */
    ...
    void copyHelper(Node *dst, Node *src);
   ...
   */
    ...
    void buildTreeHelper(Node* n, stats & s, pair<int,int> ul, pair<int,int> lr);
    pair<pair<int,int>, pair<int,int>> splitHelper(stats & s,pair<int,int> ul, pair<int,int> lr);
    RGBAPixel pixelHelper(Node* n,int i,int j);
   
   void pruneHelper(Node* root, double pct, int tol);

   double countTol (Node* root, RGBAPixel & p, int tol);

   double countLeaf (Node* root);

   bool computeD(RGBAPixel & p1, RGBAPixel & p2, int tol);
   
    double counter (Node* root);
